...
const url_api = "http://192.168.1.247:8899/cn-branch-api";


const url_salefile_get = url_api + '/v1/salefile_get';
const url_saletra_get = url_api + '/v1/saletra_get';
const url_cn_branch_pre_job_get = url_api + '/v1/Cn_Branch_Pre_Job_Get';
const url_cn_branch_pre_job_create = url_api + '/v1/Cn_Branch_Pre_Job_Create';
const Cn_Branch_Lov_Get = url_api + '/v1/Cn_Branch_Lov_Get';
...
firebase.auth().onAuthStateChanged(function (user) {
    ...
    if (user) {
        ...
        $.init = async function () {
            ...
            var branch = [
                { "id": "", "text": "-- เลือกสาขา --" },
                { "id": "VSK", "text": "VSM & VSK : ว.ศิริกาญ" },
                { "id": "VSF", "text": "VSF : ว.ศิริกาญ 2" },
                { "id": "KLH", "text": "KLH : คลองหลวง" },
                { "id": "LLK", "text": "LLK : ลำลูกกา" },
                { "id": "NWM", "text": "NWM : นวมินทร์" },
                { "id": "LKS", "text": "LKS : คลองสามวา (เลียบคลองสอง)" },
                { "id": "SNK", "text": "SNK : ศรีนครินทร์" }
            ];

            $.each(branch, function (index, item) {
                $("#branch").append("<option value='" + item.id + "' >" + item.text + "</option>");
            });
